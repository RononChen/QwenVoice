...
- `Sources/iOS/Studio/IOSStudioInlinePlayerCard.swift` — completion-state mini player with `arrow.down.to.line` Download + Dismiss buttons, soft drop shadow per design notes (`0 2 10 / 0.22`).
...
- Composer is borderless 22pt SF Pro Display weight medium with letter-spacing -0.22pt; meta + counter row sits flush below. Composer height is **fixed at 320 pt** — true `flex: 1` is the one open item, blocked on SwiftUI `TextEditor` ignoring `.frame(maxHeight: .infinity)` regardless of `.layoutPriority` placement. Proper fix is a `UIViewRepresentable<UITextView>` wrapper with `intrinsicContentSize` returning `UIView.noIntrinsicMetric`.
