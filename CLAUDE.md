...
**Known follow-ups (non-blocking, not active regressions):**

1. **`custom/warm` streaming engagement intermittent.** 1/6 samples in the Phase 3 bench fell back to file playback (autoplay 11 106 ms, fired AFTER `Final File Ready` at 10 874 ms). Suspect race between the previous cold session's `completeStreamingPreview` teardown and the warm session's first chunk arrival in `handleGenerationChunk`. The fallback still produces audible output, just without the perceived-speed gain on that one generation. Repro rate appears low; sample size too small to characterize.
2. **Cold-cell audio runs slightly louder than baseline.** custom/cold and clone/cold show +1 to +3 dB RMS above baseline mean (consistent across n=3 clone/cold samples). No clipping verified (0 saturated samples across all bench files). Likely the `AVAudioFile(forWriting:)`-based `IncrementalPCM16WAVFileWriter` (streaming path) producing slightly different output than the hand-coded `AtomicPCM16WAVWriter` (non-streaming path). Sub-baseline-stdev for most cells; investigate if user reports loudness regressions.
