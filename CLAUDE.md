...
1. **Warm-after-cold streaming engagement race — root cause + partial fix in `6c2ea52`.** Diagnostic signposts (`Chunk Received`, `Live Session Start`, `Live Engine Play`, `Session Completed Recorded`, `Switch To File Playback`) added at the streaming state transitions in `AudioPlayerViewModel` made the bug observable. Root cause: `teardownLivePlayback(clearSession: true)` calls `stopLivePlayback` which `.reset()`s the `AVAudioEngine` but leaves the `liveEngine` and `livePlayerNode` references non-nil. On the next session's first chunk, `appendLiveChunk`'s `if liveEngine == nil || livePlayerNode == nil` guard is FALSE, so `configureLiveEngine` doesn't rebuild the graph; then `attemptLivePlay`'s `liveEngine.start()` throws and the catch block silently swallows it. Fix: also nil out `liveEngine` and `livePlayerNode` in the clearSession block (`AudioPlayerViewModel.swift:1057`). Verified 3/3 custom-warm samples and 1/1 design-warm samples now engage streaming cleanly post-fix (autoplay 4-5 s BEFORE Final File Ready). **Residual edge case**: clone/warm still occasionally falls back (1/1 in the post-fix run) — different race involving cold-drain timing vs warm session swap, NOT the engine-retention bug fixed here. Suspect `liveFinalFilePath` getting set by cold's deferred `completeStreamingPreview` after warm's `attemptLivePlay` already checked it. Tracked as a future Phase 5.
