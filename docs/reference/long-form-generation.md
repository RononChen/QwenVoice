...
关于 13,000–18,000 字叙事文本及保持音调的语速控制，见
[macOS 长篇叙事与语速控制评估](long-form-narration-tempo-assessment.md)。macOS 已实现手填 `0.01–2.50`、
两位小数、默认 `1.00` 的语速控制；iOS 仍不在本轮范围内。
...
1. 用户一次提交完整文稿。
2. 超过 200 个字符即进入长文本路径；`SpokenTextPlanner` 与 `LongFormPlanner` 按段落、中文和其他语言句末标点规划分段，每段使用保守的 200 token 上限。
3. 分段按顺序、非流式生成到任务私有工作区，并逐段执行 Fast QC；引擎检查与 App 落盘复检使用同一份分段文本计算标点停顿预算，避免把正常叙事停顿误判为音频中断。
4. `BoundedLongFormAssembler` 在每两个相邻分段之间插入固定 300 毫秒静音，再用固定大小 PCM 块合并并原子发布一个 24 kHz、单声道 PCM16 WAV。
5. 语速不为 `1.00` 时，`PitchPreservingSpeechRateProcessor` 调用 FFmpeg `atempo`，对已经合并并包含停顿的完整 WAV 只处理一次；校验隐藏暂存 WAV 后原子替换成品。`1.00` 完全旁路，不启动 FFmpeg，也不重编码。
6. 完整 WAV 接受后只写入一条 History 成品记录（包含语速和处理后时长），然后清理全部分段过程文件。
...
- 每个窗口尽量填满到 200 token，再回退到窗口内最靠后的段落、句子（包含中日韩标点）、分号/冒号或安全从句边界；只有同一位置存在多种边界时才按语义优先级判定，最后才回退到空白或字素。
...
语速处理只针对合并后的成品。FFmpeg 从原成品流式读取并写入隐藏暂存 WAV，不把整条音频载入内存；验证后
原子替换原成品。成功、失败或取消都会清理暂存文件，不长期保留“原始版”和“变速版”两个完整副本。分段仍然
最多只保留一份已接受 WAV。
...
普通开发构建仍可解析本机 FFmpeg，用于快速验证，但正式发布绝不复制 Windows 可执行文件或 Homebrew 的完整 GPL
构建。发布脚本从固定 FFmpeg 8.0.3 官方源码构建 arm64 `ffmpeg-vocello` 独立 helper，只开放本地 WAV、PCM16 和
`atempo` 所需能力，单独签名后放入 `Contents/Helpers`。完整 LGPL、上游许可证和构建身份随 App 保存，精确对应
源码、分离签名与构建身份作为同一 GitHub Release 资产并进入 SBOM 和发布证据。

组件合同、复现命令、能力白名单和仍需执行的 Developer ID 公证候选版检查见
