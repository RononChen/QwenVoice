...
> 当前状态：**进行中**。公司主体、组织账号、统一品牌和收费模式已经确认；产品工作名采用 `Sonafolio`，
> 中文辅助名采用“声卷 / 聲卷”，首版 ASO 和 Logo 方向已经形成。正式商标/名称检索、原创 Logo 和最终 Bundle ID
> 尚未冻结；开发者账号注册按维护者当前安排延后。

阶段内的当前决策记录见[macOS 产品身份表](macos-product-identity-sheet.md)。
...
- 商业发行主体为北京艺文美达文化传播有限公司，并以组织身份加入 Apple Developer Program；
- Developer ID 商店外版本和 Mac App Store 版本使用同一个产品品牌；当前没有官网不影响品牌统一，
  以后如建立官网也沿用同一品牌；
- 产品工作名采用 `Sonafolio`，中文辅助名采用“声卷 / 聲卷”，正式发行前仍须完成近似商标、App Store、
  域名和社交账号检索。
...
- 验证本地 Whisper 字幕模型下载、SHA-256 校验、最终变速 WAV 对齐、同名 SRT 原子写入和失败不回滚 WAV；
- 以 Apple Silicon Mac + 16 GB 统一内存为商业版最低验收设备，不承诺 8 GB 商业支持。
