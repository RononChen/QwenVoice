...
- **No inline PCM preview by default.** On physical iOS the streaming chunk's `previewAudio` PCM is
  skipped (`NativeStreamingPreviewDataPolicy` → `.skip`); override with
  `QWENVOICE_STREAMING_PREVIEW_DATA=on`. (`SemanticTypes.swift` / `NativeStreamingSynthesisSession.swift`.)
...
bump. Procedure (OPTIMIZATION.md §E, CLAUDE.md "SPM dependencies"): throwaway branch → bump all pin
...
- **No inline PCM preview on physical iOS by default** — keep `QWENVOICE_STREAMING_PREVIEW_DATA` off
  in shipping; it's a debug/inspection lever only.
