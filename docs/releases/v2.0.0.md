...
- **Notarized + signed DMG on every GitHub Release.** First launch opens with a double-click — no right-click bypass, no `xattr -dr com.apple.quarantine`, no "unidentified developer" warning. The DMG carries a stapled Apple notarization ticket and is signed by `Developer ID Application: PATRICE DERY (FK2D8X36G2)`.
...
- **Agent guide** lives at [`CLAUDE.md`](https://github.com/PowerBeef/QwenVoice/blob/v2.0.0/CLAUDE.md) (legacy `AGENTS.md` retired). Single source of truth for repo conventions.
- **`scripts/build.sh`** is the canonical local entrypoint (`debug`, `run`, `release`, `clean`). Single-resident build policy: at most one Debug `.app` and one Release `.app/.dmg` live under `build/` at a time.
- **CI is scoped to release packaging only.** No tests, no benches, no smoke runs on CI. Smoke + bench + perceptual-review layers are all local, driven via `scripts/uitest.sh` + the runbooks under `docs/reference/`. `scripts/check_project_inputs.sh` enforces the prohibited-paths list.
- **Runtime data folders are configuration-aware:**
  - Debug → `~/Library/Application Support/QwenVoice-Debug/`
  - Repo-local Release (built via `scripts/release.sh` and run from `build/Release/`) → `~/Library/Application Support/QwenVoice-Release-Local/<release-data-id>/`
  - Installed/public Release (in `/Applications/`) → `~/Library/Application Support/QwenVoice/`
  - The `QWENVOICE_APP_SUPPORT_DIR` env var overrides everywhere.
- **No new tests or GitHub Actions workflows** without an explicit maintainer decision, per the carve-out in `CLAUDE.md`. Testing strategy is documented in [`docs/reference/testing-overview.md`](https://github.com/PowerBeef/QwenVoice/blob/v2.0.0/docs/reference/testing-overview.md).
...
- `spctl --assess --type install -vv Vocello-macos26.dmg` should print `accepted` with `source=Notarized Developer ID` and `origin=Developer ID Application: PATRICE DERY (FK2D8X36G2)`.
